...
(function () {
  ...
  function frame(now) {
    ...
    // upright idle: gentle breathing only — NO sway, NO lean, face forward
    pose.hips = { ang: 0 };
    pose.spine = { ang: 0, sx: 1, sy: 1 + a * 0.012 * br };
    pose.head = { ang: a * 0.012 * Math.sin(TAU * t / 5.0), sy: 1 + a * 0.01 * br };
    pose.upperArmL = { ang: a * 0.02 * Math.sin(TAU * t / 5.5) };
    pose.upperArmR = { ang: a * 0.02 * Math.sin(TAU * t / 5.5 + Math.PI) };
    pose.lowerArmL = { ang: a * 0.025 * Math.sin(TAU * t / 5.5) };
    pose.lowerArmR = { ang: a * 0.025 * Math.sin(TAU * t / 5.5 + Math.PI) };
